		...
		if bid.(float64) > 0 && ask.(float64) > 0 {
			...
			if profitLockStart != 0 {
				...
				if profitLocked == 1 && (thresHold == 0 || (bid.(float64) >= oldHighBid)) {
					...
					lockProceed = RoundDown((actualQty*sellPrice), 8) - ReduceByPercent((actualQty*sellPrice), txnFee)
			...
			if manualExit == 0 && bid.(float64) <= thresHold {
				fmt.Println("..JOB ID: ", idSellWorker, ". Threshold Breached! Determining Best Sell Price...")
			...
			fmt.Println("SELL WORKER ANALYSIS FOR JOB ID:", idSellWorker, "By Worker ID", workID)
